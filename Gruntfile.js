...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    compress: {
      ...
      finalWindowsApp: {
        options: {
          archive: '<%= config.distWin %>/flickrDownloader.zip'
        },
        files: [{
          expand: true,
          cwd: '<%= config.tmp %>',
    ...
    rename: {
      macApp32: {
        files: [{
          ...
          dest: '<%= config.distMac32 %>/flickrDownloader.app'
        }]
      },
      macApp64: {
        files: [{
          src: '<%= config.distMac64 %>/node-webkit.app',
          dest: '<%= config.distMac64 %>/flickrDownloader.app'
        }]
      },
      zipToApp: {
        files: [{
  ...
  grunt.registerTask('chmod32', 'Add lost Permissions.', function () {
    var fs = require('fs'),
      path = config.distMac32 + '/flickrDownloader.app/Contents/';
  ...
  grunt.registerTask('chmod64', 'Add lost Permissions.', function () {
    var fs = require('fs'),
      path = config.distMac64 + '/flickrDownloader.app/Contents/';
    fs.chmodSync(path + 'Frameworks/node-webkit Helper EH.app/Contents/MacOS/node-webkit Helper EH', '555');
    fs.chmodSync(path + 'Frameworks/node-webkit Helper NP.app/Contents/MacOS/node-webkit Helper NP', '555');
    fs.chmodSync(path + 'Frameworks/node-webkit Helper.app/Contents/MacOS/node-webkit Helper', '555');
    fs.chmodSync(path + 'MacOS/node-webkit', '555');
  ...
  grunt.registerTask('createWindowsApp', 'Create windows distribution.', function () {
    ...
    concat([
      'buildTmp/nw.exe',
      'buildTmp/app.nw'
    ], 'buildTmp/flickrDownloader.exe', function () {
      var fs = require('fs');
  ...
  grunt.registerTask('dmg', 'Create dmg from previously created app folder in dist.', function (osBits) {
    var done = this.async();
    var createDmgCommand = 'resources/mac/package.sh "flickrDownloader"' + ' "' + osBits + '"' ;
